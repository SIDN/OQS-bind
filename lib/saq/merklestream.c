...
SAQ_merkle_stream_finalize_until(SAQ_merkle_stream_t *s, uint64_t *until) {
    ...
    while (l != NULL && (until == NULL || nodes_added < *until)) {
        SAQ_merkle_node_t *n = NULL;
        if (SAQ_list_get_data(l, 0, (void **)&n) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
        s->tree_nodes[nodes_added] = n;
        nodes_added++;
        last = n;
        if (SAQ_list_get_next(l, &l) != SAQ_SUCCESS) {
	    free(s->tree_nodes);
            s->tree_nodes = NULL;
            return SAQ_FAILURE;
        }
...
SAQ_merkle_stream_get_proof(SAQ_merkle_stream_t *s, uint64_t i,
    ...
    SAQ_authentication_node_t *ap = NULL;
    ...
    hash_cleanup:
        ...
        SAQ_authentication_path_append(&proof, ap);
        cur_node = parent;
        if (SAQ_merkle_node_get_parent(cur_node, &parent) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
...
SAQ_merkle_stream_byte_size(SAQ_merkle_stream_t *s, uint64_t *size) {
    ...
    SAQ_list_t *c = s->data_nodes;
    uint64_t sum = 0;
    c = s->data_nodes;
    while (c != NULL) {
        uint64_t len = 0;
        SAQ_merkle_node_t *n = (SAQ_merkle_node_t *)c->data;
        if (SAQ_merkle_node_byte_size(n, &len) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
        sum += len;
        if (SAQ_list_get_next(c, &c) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
    }
...
SAQ_merkle_stream_serialize(SAQ_merkle_stream_t *s, unsigned char *out,
    ...
    *out_len = len;
    ...
    uint64_t count = htonl(s->tree_node_count);
    memcpy(out, &count, sizeof(uint64_t));
    out += sizeof(uint64_t);
    len -= sizeof(uint64_t);

    count = htonl(s->data_node_count);
    memcpy(out, &count, sizeof(uint64_t));
    out += sizeof(uint64_t);
    len -= sizeof(uint64_t);
...
SAQ_merkle_stream_deserialize(unsigned char *in, uint64_t in_len,
    ...
    // finalized to before serialization.
    uint64_t tree_node_count = ntohl(*(uint64_t *)in);
    in += sizeof(uint64_t);
    in_len -= sizeof(uint64_t);

    uint64_t data_node_count = ntohl(*(uint64_t *)in);
    in += sizeof(uint64_t);
    in_len -= sizeof(uint64_t);
    // TODO remove structure mallocs in other deserializes and use `new` if able
