...
SAQ_authentication_path_get_node(SAQ_authentication_path_t *ap,
    ...
    return SAQ_list_get_data(ap, 0, (void **)out);
}
...
SAQ_authentication_path_prove(unsigned char *data, uint64_t data_len,
    ...
    SAQ_authentication_path_t *cur_path = ap;
    ...
    while (cur_path != NULL) {
        SAQ_authentication_path_t *nxt_path = NULL;
        if (SAQ_authentication_path_get_next(cur_path, &nxt_path)
            != SAQ_SUCCESS) {
            goto err;
        }
        ...
        cur_path = nxt_path;
        cur_node = NULL;
        if (SAQ_authentication_path_get_node(cur_path, &cur_node)
              != SAQ_SUCCESS) {
            goto err;
        }
    }
...
SAQ_authentication_node_serialize(SAQ_authentication_node_t *n,
    ...
    uint64_t h_len = htonl(n->sibling_hash_len);
...
SAQ_authentication_node_deserialize(unsigned char *in, uint64_t in_len,
    ...
    uint64_t hash_len = ntohl(*(uint64_t *)in);
    in += sizeof(uint64_t);
...
SAQ_authentication_path_byte_size(SAQ_authentication_path_t *ap,
    ...
    uint64_t byte_sum = 0;
    SAQ_authentication_path_t *c = ap;
    while (c != NULL) {
        uint64_t an_len = 0;
        SAQ_authentication_node_t *n = NULL;
        if (SAQ_authentication_path_get_node(c, &n) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
        ...
        byte_sum += an_len;
        if (SAQ_authentication_path_get_next(c, &c) != SAQ_SUCCESS) {
            return SAQ_FAILURE;
        }
    }
...
SAQ_authentication_path_deserialize(unsigned char *in, uint64_t in_len,
    ...
    SAQ_authentication_path_t *ap;
    SAQ_authentication_path_init(&ap);
